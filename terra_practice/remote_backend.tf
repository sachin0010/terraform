...
terraform refresh
terraform state pull > terraform.tfstate     -  pull into local from s3
terraform state pull | head

* now only one user can  add/modify anything due to locking the state. if another person will he will get error like lock id generated by this user
******   once lock id will remove from dynamedb table  after completing apply then only another person can modify ***********
